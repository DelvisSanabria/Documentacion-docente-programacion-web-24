        ...
        if(isset($_POST['correo']) && isset($_POST['clave'])){
            ...
            if(!empty($_POST['correo']) && !empty($_POST['clave'])){
                ...
                if($conn->connect_error){
                    ...
                    // Validamos la informacion que viene del formulario para evitar que un atacante inyecte codigo malicioso (SQL Injection)
                    // Para ello usamos regex
                    if(preg_match("/^[a-zA-Z0-9._]+@[a-zA-Z0-9._]+\.[a-zA-Z]{2,}$/",$correo) && preg_match("/^[\w.!@+]+$/", $clave)){
                        // Creamos la consulta para buscar en la base de datos al usuario en base al correo indicado
                        $consulta = "SELECT nombre, apellido, url_foto, rol, clave FROM usuarios WHERE correo = '$correo'";
                        ...
                        echo "<h3>¿A donde vas?, ¡Tramposo 😠!</h3>";
                        // Redireccionamos al usuario al formulario de registro despues de 3 segundos
                        header("refresh:3;url=login_front.php");
                    }
                }
